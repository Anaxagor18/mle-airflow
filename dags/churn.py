...
import sqlalchemy
from sqlalchemy import MetaData, Table, Column, String, Float, Integer, DateTime, Boolean, Date, UniqueConstraint, inspect
...
def prepare_churn_dataset():
    ...
    def create_table() -> None:
        ...
        users_churn = Table('users_churn', metadata,
                       ...
                       UniqueConstraint('customer_id'), # Уникальное ограничение для customer_id
                      )
        if not inspect(db_conn).has_table('users_churn'):
            metadata.create_all(bind=db_conn)
